    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Manage all orders</p>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={refreshStatuses} disabled={refreshing} className="h-9 text-xs font-semibold">
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-muted/50">
                ...
                <TableHead className="font-semibold text-xs">Set Status</TableHead>
                <TableHead className="font-semibold text-xs text-right">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {orders.length === 0 && (
                <TableRow><TableCell colSpan={8} className="text-center text-muted-foreground py-12">No orders found</TableCell></TableRow>
              )}
              {orders.map((o) => {
                  ...
                  <TableRow key={o.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <TableCell className="text-xs">
                      <div className="space-y-0.5">
                        ...
                        <div className="text-muted-foreground text-[11px]">${balanceUSD.toFixed(2)} / ₹{balanceINR.toFixed(2)}</div>
                      </div>
                    </TableCell>
                    ...
                    <TableCell>
                      <Badge variant="outline" className={`font-semibold capitalize text-[11px] ${statusColors[o.status] || ""}`}>{o.status}</Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      {(o.status === "failed" || o.status === "pending") && (
                        <Button variant="outline" size="sm" onClick={() => refundOrder(o)} className="h-7 text-xs">Refund</Button>
                      )}
                    </TableCell>
                  </TableRow>
                );
        ...
        <div className="flex items-center justify-between pt-1">
          ...
          <div className="flex items-center gap-1">
            ...
            {Array.from({ length: Math.min(totalPages, 7) }, (_, i) => {
              ...
              return (
                <Button key={pageNum} variant={page === pageNum ? "default" : "outline"} size="sm" onClick={() => setPage(pageNum)} className="h-8 w-8 p-0 text-xs font-semibold">
                  {pageNum + 1}
                </Button>
              );
            })}
